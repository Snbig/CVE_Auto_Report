...
def send_cve_message_to_telegram(cve_data):
    try:
        ...
        url = f'https://api.telegram.org/bot{telegram_bot_token}/sendMessage?chat_id={telegram_chat_id}&message_thread_id={telegram_thread_id}&text={message}&parse_mode=MarkdownV2'
...
prompts = {
    "tags" : "چند تا برچسب به انگلیسی برای آسیب پذیری {{ID}} بنویس در برچسب از نوشتن امتیاز خودداری بکن و فقط برچسب های کلی را بنویس هر بچسب را داخل [] بنویس ",
    "chart" : "با توجه به به آسیب پذیری با شناسه {{ID}} اگر در مورد دیتابیس بود عدد 17 را نمایش بده اگر در مورد سیستم عامل بود عدد 16 و 17 و 2 و اگر در مورد تجهیزات بود مثل سیسکو ، فورتیگیت ، استوریج و .. عدد 16 و 4 و اگر در مورد سایت، وب اپلیکیشن و وب سرور بود 17 و 18 را در خروجی نمایش بده و حتما اعداد را داخل [] بگذار",
    "title" : "یک عنوان به زبان فارسی برای آسیب پذیری با شناسه {{ID}} بنویس با توجه به منابع {{REF}} و حتما آن را داخل [] قرار بده",
    "info" : "یک توضیح کامل در مورد آسیب پذیری با شناسه {{ID}} با منابع {{REF}} به زبان فارسی بنویس و هیچ توضیحی در خصوص راهکار امن سازی نده و سوال هم نپرس",
    "remedition" : "یک راهکار امن سازی برای آسیب پذیری با شناسه {{ID}} بنویس با توجه به منابع {{REF}} و اگر در ورژن های بعدی آسیب پذیری رفع شده بود ورژن دقیق امن را در آخر بنویس و در متن هم سوال نپرس",
}
